...
test.describe('PartnerMatrix Login Flow', () => {
  ...
  test('should allow a user to log in successfully on PartnerMatrix', async ({ page }) => {
   ...
   const loginButton = page.getByRole('button', { name: 'Login' });
    ...
    console.log('Awaiting response from server and check redirect landing page...');
    // page.wait_for_url("*/organic-traffic/website-analysis/website-overview")/

    await expect(page).toHaveURL('https://data.partnermatrix.com/organic-traffic/website-analysis/website-overview',{waitUntil: 'networkidle'});
  });
